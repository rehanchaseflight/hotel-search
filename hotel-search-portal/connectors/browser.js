...
async function searchBrowserSource(source,search){
 ...
 try{
  ...
  const rows=await page.locator(cfg.result_row_selector||'body').evaluateAll((nodes,c)=>{const get=(root,sel)=>{if(!sel)return'';const el=root.querySelector(sel);return el?(el.innerText||el.textContent||'').replace(/\s+/g,' ').trim():''};return nodes.slice(0,Number(c.max_results)||500).map((root,i)=>({index:i,hotel:get(root,c.hotel_selector),room:get(root,c.room_selector),view:get(root,c.view_selector),board:get(root,c.board_selector),price:get(root,c.price_selector),currency:get(root,c.currency_selector),cancellation:get(root,c.cancellation_selector),available:get(root,c.availability_selector)}))},cfg);
  const results=rows.map((r,i)=>({id:`${source.id}-${i}`,supplier:source.name,hotel:r.hotel||'Hotel',room:r.room||'',view:r.view||'',board:r.board||search.board,price:number(r.price),currency:r.currency||cfg.default_currency||'',cancellation:r.cancellation||'',availability:r.available||'Available',raw:r})).filter(r=>r.hotel!=='Hotel'||Number.isFinite(r.price));

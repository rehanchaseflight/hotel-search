...
function selector(value, fallback) { return value || fallback; }
...
async function firstText(page, selectorValue) {
  if (!selectorValue) return '';
  try { return text(await page.locator(selectorValue).first().innerText({ timeout: 2500 })); }
  catch { return ''; }
}
...
async function searchBrowserSource(source, search) {
  ...
  let password;
  try { password = decrypt(source.site_password_enc); }
  catch (e) { return { configured: true, results: [], error: `Credential decryption failed: ${e.message}` }; }
  ...
  try {
    ...
    const rows = await page.locator(cfg.result_row_selector || 'body').evaluateAll((nodes, c) => {
      const get = (root, sel) => { if (!sel) return ''; const el = root.querySelector(sel); return el ? (el.innerText || el.textContent || '').replace(/\s+/g, ' ').trim() : ''; };
      return nodes.slice(0, Number(c.max_results) || 300).map((root, i) => ({
        index: i,
        hotel: get(root, c.hotel_selector), room: get(root, c.room_selector), board: get(root, c.board_selector),
        price: get(root, c.price_selector), currency: get(root, c.currency_selector), cancellation: get(root, c.cancellation_selector),
        bookingUrl: root.querySelector(c.booking_url_selector || 'a[href]')?.href || '', image: root.querySelector(c.image_selector || 'img')?.src || ''
      }));
    }, cfg);
    ...
    const results = rows.map((r, i) => ({
      ...
      hotel: r.hotel || 'Hotel', room: r.room || '', board: r.board || search.board,
      price: number(r.price), currency: r.currency || cfg.default_currency || '',
      cancellation: r.cancellation || '', bookingUrl: r.bookingUrl || '', image: r.image || '', raw: r
    })).filter(r => r.hotel !== 'Hotel' || Number.isFinite(r.price));

...
async function searchRezLive(page, search, cfg) {
  const agent = sourceSelector(cfg.agent_code_selector, [
    'input[name*="agent" i]',
    'input[id*="agent" i]',
  ...
  if (agent && cfg._agentCode) await page.locator(agent).first().fill(cfg._agentCode);
  if (username) await page.locator(username).first().fill(cfg._username);
  if (password) await page.locator(password).first().fill(cfg._password);

  const loginButton = sourceSelector(cfg.login_button_selector, [
    'button:has-text("Login")',
    'input[type="submit"]',
    'button[type="submit"]'
  ]);
  if (loginButton) await page.locator(loginButton).first().click();
  else if (password) await page.locator(password).first().press('Enter');
  ...
  if (searchUrl) {
    ...
    const checkout = sourceSelector(cfg.checkout_selector, [
      ...
      'input[id*="checkout" i]',
      'input[id*="check-out" i]'
    ]);
    const guests = sourceSelector(cfg.guests_selector, [
      'input[name*="guest" i]',
      'input[id*="guest" i]'
    ]);
    const rooms = sourceSelector(cfg.rooms_selector, [
      'input[name*="room" i]',
      'input[id*="room" i]'
    ]);
    ...
    if (searchButton) await page.locator(searchButton).first().click();
    else throw new Error('RezLive search button could not be detected');
  }

  if (cfg.results_wait_for_selector) {
    await page.locator(cfg.results_wait_for_selector).first().waitFor({
      state: 'visible', timeout: Number(cfg.results_timeout_ms) || 20000
    });
  } else {
    await page.waitForTimeout(Number(cfg.results_wait_ms) || 4000);
  }

  return extractRezLiveRates(page, cfg);
}

function sourceSelector(explicit, fallbacks) {
  return explicit || fallbacks.find(Boolean) || null;
}
...
async function extractRezLiveRates(page, cfg) {
  ...
  const rows = await page.evaluate(() => {
    ...
    const availableNodes = [...document.querySelectorAll('body *')].filter(el => {
      ...
      return t.length >= 40 && t.length <= 2500 && /\bavailable\b/i.test(t) && priceRe.test(t);
    });
...
async function extractConfiguredRows(page, cfg) {
  return page.locator(cfg.result_row_selector).evaluateAll((nodes, c) => {
    ...
    return nodes.slice(0, Number(c.max_results) || 500).map((root, index) => ({
      index,
      ...
      cancellation: get(root, c.cancellation_selector),
      availability: get(root, c.availability_selector)
    }));
  }, cfg);
}
...
async function searchBrowserSource(source, search) {
  ...
  try {
    ...
    if (cfg.preset === 'rezlive') {
      rows = await searchRezLive(page, search, {
        ...cfg,
        _agentCode: source.agent_code || '',
        _username: source.site_username,
        _password: password
      });
    } else {
      ...
      if (cfg.login_button_selector) await page.locator(cfg.login_button_selector).click();
      else if (cfg.password_selector) await page.locator(cfg.password_selector).press('Enter');
      await page.waitForLoadState('domcontentloaded').catch(() => {});
      ...
      if (cfg.results_wait_for_selector) await page.locator(cfg.results_wait_for_selector).first().waitFor({state:'visible',timeout:Number(cfg.results_timeout_ms)||20000});
      else if (cfg.results_wait_ms) await page.waitForTimeout(Number(cfg.results_wait_ms));

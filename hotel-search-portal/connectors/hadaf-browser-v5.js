...
function parseDisplayedPrice(text, def = 'AED') {
  ...
  if (!m) return null;
  const q = m[0].replace(/,/g, '').match(/[0-9]+(?:\.[0-9]{1,2})?/);
  if (!q) return null;
  const n = Number(q[0]);
  if (!(n > 0)) return null;
...
function board(text) {
  const m = clean(text).match(BOARDS);
  return m ? clean(m[0]) : '';
}

function cancel(text) {
  const m = clean(text).match(/Non[- ]?refundable|Free Cancellation|Refundable/i);
  return m ? clean(m[0]) : '';
}

function parseRoom(text) {
  let x = clean(text)
    .replace(/^.*?Pax:\s*\d+A\d+C\s*/i, '')
    .replace(/\s+(?:Non[- ]?refundable|Free Cancellation|Refundable|Package Rate|Show All Room Types|Live Search Results)\b.*$/i, '');
  const m = x.match(/^(.+?)\s*-\s*(?:Room Only|Breakfast Included|Bed and Breakfast|Half Board|Full Board|All Inclusive)\b/i);
  return clean(m ? m[1] : x);
}

function dedupe(rows) {
  const seen = new Set();
  return rows.filter((r) => {
    const key = [norm(r.hotel), norm(r.room), norm(r.board), norm(r.cancellation), Number(r.price).toFixed(2), norm(r.currency)].join('|');
...
async function findLoginFrame(page) {
  for (const frame of page.frames()) {
    if (await frame.locator('#tbUserName').count().catch(() => 0)) return frame;
  }
  return null;
}
...
async function login(page, source, password, cfg) {
  ...
  if (await terms.count() && !(await terms.isChecked().catch(() => false))) {
...
async function searchForm(page) {
  ...
  while (Date.now() < end) {
    for (const frame of page.frames()) {
      const city = frame.locator('#lpPannel_txtV5City');
      const from = frame.locator('#lpPannel_txtFromDate');
      if (await city.count().catch(() => 0) && await from.count().catch(() => 0)) return frame;
    }
...
async function chooseDestination(frame, value) {
  ...
  await input.fill(clean(value));
  await frame.waitForTimeout(1200);
  const pattern = new RegExp(escapeRegex(clean(value)), 'i');
  const options = frame.locator('li:visible, [role="option"]:visible').filter({ hasText: pattern });
...
async function fillSearch(frame, search) {
  ...
  if (await nationality.count()) {
    await nationality.selectOption({ label: String(search.country || 'United States of America') }).catch(() => {});
  }
  ...
  if (search.hotel_name) {
    ...
    if (await hotel.count()) {
      const name = String(search.hotel_name);
      await hotel.evaluate((el, value) => {
        el.value = value;
...
async function clickSearch(frame) {
  const searchLink = frame.locator('a#lpPannel_btnModifySearch').first();
  if (!await searchLink.count()) throw new Error('Hadaf search button not found');
  await searchLink.click({ timeout: 10000, noWaitAfter: true });
}
...
function flattenJson(value, out = []) {
  ...
  if (value.n && value.rlst) {
    for (const group of value.rlst || []) {
      for (const rate of group.rdlst || []) {
        out.push({
          hotel: clean(value.n),
          room: clean(rate.rt),
          board: clean(rate.mp),
          cancellation: /^(N)$/i.test(String(rate.p || '')) || /^(N)$/i.test(String(rate.bo || '')) ? 'Non-refundable' : '',
  ...
  for (const key of Object.keys(value)) {
    if (key !== 'rlst') flattenJson(value[key], out);
  }
  return out;
}
...
function captureJson(context, state) {
  context.on('response', async (response) => {
    try {
      if (/GetHotelsJson\.aspx/i.test(response.url()) && response.status() === 200) {
        state.json = flattenJson(JSON.parse(await response.text()), []);
        state.jsonResponse = true;
      }
    } catch (_) {}
  });
}
...
function matchJson(state, room, boardName, hotel) {
  if (!room && !boardName && !hotel) return null;
  let best = null;
  let score = -1;
  for (const item of state.json || []) {
    let value = 0;
    if (hotel && norm(item.hotel) === norm(hotel)) value += 20;
    if (room && norm(item.room) === norm(room)) value += 10;
    else if (room && (norm(item.room).includes(norm(room)) || norm(room).includes(norm(item.room)))) value += 6;
    if (boardName && norm(item.board) === norm(boardName)) value += 8;
    if (value > score) {
      score = value;
      best = item;
    }
...
function makeRate(search, best, room, boardName, price, context) {
  return {
    hotel: search.hotel_name || best?.hotel || '',
    room,
    view: '',
    board: boardName,
    cancellation: cancel(context) || best?.cancellation || '',
    price: price.price,
    currency: price.currency,
    availability: 'Available',
    supplier: 'Hadaf Holidays',
    supplierRoomCode: best?.supplierRoomCode || '',
    rateFrom: best?.rateFrom || '',
    rateTo: best?.rateTo || ''
  };
}
...
async function expandRooms(context) {
  for (const page of context.pages()) {
    for (const frame of page.frames()) {
      const locator = frame.getByText(/Show All Room Types/i);
      const count = await locator.count().catch(() => 0);
      for (let i = 0; i < count; i++) {
        if (await locator.nth(i).isVisible().catch(() => false)) {
          await locator.nth(i).click({ noWaitAfter: true, timeout: 3000 }).catch(() => {});
        }
      }
    }
  }
      ...
      for (let i = 0; i < count; i++) {
        ...
        const info = await link.evaluate((el) => {
          ...
          for (let depth = 0; node && depth < 8; depth++, node = node.parentElement) {
            ...
            if (prices.length === 1 && boardRe.test(text) && text.length < 600) {
              best = text;
              break;
            }
          }
          return { anchor: (el.innerText || '').replace(/\s+/g, ' ').trim(), container: best };
        }).catch(() => ({ anchor: '', container: '' }));

        const text = clean(info.container || info.anchor);
        const price = parseDisplayedPrice(text);
        if (!price) continue;

        let room = parseRoom(text);
        let boardName = board(text);
        const pax = text.match(/Pax:\s*\d+A\d+C\s+(.+)/i);
        if (pax) room = parseRoom(pax[1]);
        ...
        if (room && boardName) out.push(makeRate(search, best, room, boardName, price, text));
      }
    }
  }

  return dedupe(out)
    .filter((item) => item.price > 0 && item.room && item.board)
...
async function searchHadafSource(source, search) {
  ...
  try {
    ...
    return { configured: true, results: [], error: `Credential decryption failed: ${error.message}` };
  }
  ...
  try {
    ...
    const state = { json: [], jsonResponse: false };
    ...
    await page.goto(cfg.search_page_url || SEARCH_PAGE, { waitUntil: 'domcontentloaded', timeout: 30000 }).catch(() => {});
    ...
    while (Date.now() < end) {
      await blocked(page);
      if (state.jsonResponse && state.json.length) break;
      await new Promise((resolve) => setTimeout(resolve, 1000));
    }

    if (!state.jsonResponse && !context.pages().some((p) => /HotelResults\.aspx/i.test(p.url()))) {
      throw new Error(`Hadaf search did not return results. URL: ${page.url()}`);
    }

    await page.waitForTimeout(Number(cfg.json_settle_wait_ms) || 10000);
    await expandRooms(context);

    const results = await extractRendered(context, cfg, search, state);
    if (!results.length) {
      throw new Error(`Hadaf returned results but no complete priced rate elements were found. JSON rates: ${state.json.length}. URL: ${page.url()}`);
    }
    ...
    return {
      ...
      error: error.name === 'TimeoutError'
        ? `Hadaf browser timed out at Playwright operation: ${error.message}`
        : error.message
    };
  } finally {
...
async function healthHadafSource(source) {
  return {
    configured: !!(source.login_url && source.site_username && source.site_password_enc),
    live: true,
    error: null
  };
}

...
function parseDisplayedPrice(text, def = 'AED') {
  ...
  const q = m[0].replace(/,/g, '').match(/[0-9]+(?:\.[0-9]{1,2})?/); if (!q) return null;
  const n = Number(q[0]); if (!(n > 0)) return null;
  const cm = m[0].match(/AED|SAR|USD|EUR|GBP|PKR|US\$|\$/i);
  return { price: n, currency: cm ? cm[0].toUpperCase() : def };
}
...
function parseRoom(text) {
  const x = clean(text).replace(/^.*?Pax:\s*\d+A\d+C\s*/i, '');
  const m = x.match(/\b((?:Double|Twin|Triple|Quadruple|Quintuple|Family|Standard|Deluxe|Superior|King|Queen|Single)[A-Za-z0-9 /-]{2,100}?)\s*-\s*(?:Room Only|Breakfast Included|Bed and Breakfast|Half Board|Full Board|All Inclusive)\b/i);
  return clean(m ? m[1] : x.replace(/\s+(?:Non[- ]?refundable|Free Cancellation|Refundable|Package Rate|Show All Room Types|Live Search Results)\b.*$/i, ''));
}
function dedupe(rows) {
  const seen = new Set();
  return rows.filter((r) => { const key = [norm(r.hotel), norm(r.room), norm(r.board), norm(r.cancellation), Number(r.price).toFixed(2), norm(r.currency)].join('|'); if (seen.has(key)) return false; seen.add(key); return true; });
}
...
async function chooseDestination(frame, value) {
  ...
  const pattern = new RegExp(escapeRegex(clean(value)), 'i'); const options = frame.locator('li:visible, [role="option"]:visible').filter({ hasText: pattern });
...
async function fillSearch(frame, search) {
  ...
  await frame.locator('#lpPannel_txtFromDate').fill(date(search.checkin)); await frame.locator('#lpPannel_txtToDate').fill(date(search.checkout));
  await frame.locator('#lpPannel_txtNights').fill(String(Math.max(1, Number(search.nights || 1))));
  await frame.locator('#sel_NoOfRooms').selectOption(String(search.rooms || 1)).catch(() => {}); await frame.locator('#sel_NoOfAdult_1').selectOption(String(search.guests || 2)).catch(() => {}); await frame.locator('#sel_NoOfChild_1').selectOption(String(search.children ?? 0)).catch(() => {});
  if (search.hotel_name) { const hotel = frame.locator('#lpPannel_txtHotel').first(); if (await hotel.count()) await hotel.evaluate((el, value) => { el.value = value; el.dispatchEvent(new Event('input', { bubbles: true })); el.dispatchEvent(new Event('change', { bubbles: true })); }, String(search.hotel_name)).catch(() => {}); }
}
...
function matchJson(state, room, boardName, hotel) {
  if (!room && !boardName && !hotel) return null; let best = null; let score = -1;
  for (const item of state.json || []) { let s = 0; if (hotel && norm(item.hotel) === norm(hotel)) s += 20; if (room && norm(item.room) === norm(room)) s += 10; else if (room && (norm(item.room).includes(norm(room)) || norm(room).includes(norm(item.room)))) s += 6; if (boardName && norm(item.board) === norm(boardName)) s += 8; if (s > score) { score = s; best = item; } }
...
async function expandRooms(context) { for (const page of context.pages()) for (const frame of page.frames()) { const loc = frame.getByText(/Show All Room Types/i); const n = await loc.count().catch(() => 0); for (let i = 0; i < n; i++) if (await loc.nth(i).isVisible().catch(() => false)) await loc.nth(i).click({ noWaitAfter: true, timeout: 3000 }).catch(() => {}); } }

async function extractRendered(context, cfg, search, state) {
  const out = [];
  for (const page of context.pages()) for (const frame of page.frames()) {
    const links = frame.locator('a[href*="RatePopup"]'); const count = await links.count().catch(() => 0);
    for (let i = 0; i < count; i++) {
      ...
      const info = await link.evaluate((el) => {
        ...
        const roomRe = /(?:Double|Twin|Triple|Quadruple|Quintuple|Family|Standard|Deluxe|Superior|King|Queen|Single)[A-Za-z0-9 /-]{2,100}?\s*-\s*(?:Room Only|Breakfast Included|Bed and Breakfast|Half Board|Full Board|All Inclusive)\b/i;
        const roomFrom = (text) => { const m = text.match(roomRe); return m ? text.slice(m.index, m.index + m[0].length).replace(/\s*-\s*(?:Room Only|Breakfast Included|Bed and Breakfast|Half Board|Full Board|All Inclusive).*$/i, '').trim() : ''; };
        const get = (n) => ({ text: textOf(n), prices: textOf(n).match(priceRe) || [], rooms: roomFrom(textOf(n)), board: textOf(n).match(boardRe)?.[0] || '', cancellation: textOf(n).match(cancelRe)?.[0] || '' });
        let row = el.closest?.('tr');
        if (row) {
          const cell = el.closest('td,th'); const cellText = textOf(cell); const prev = [];
          let p = cell?.previousElementSibling; for (let j = 0; p && j < 5; j++, p = p.previousElementSibling) prev.push(get(p));
          const local = get(cell); const candidate = local.rooms || prev.find((x) => x.rooms)?.rooms || roomFrom(textOf(row));
          const localCancel = local.cancellation || prev.find((x) => x.cancellation)?.cancellation || '';
          const localBoard = local.board || prev.find((x) => x.board)?.board || textOf(row).match(boardRe)?.[0] || '';
          const rowPrices = textOf(row).match(priceRe) || [];
          if (candidate) return { text: cellText || textOf(row), room: candidate, board: localBoard, cancellation: localCancel, priceCount: rowPrices.length };
        }
        let node = el; let chosen = '';
        for (let depth = 0; node && depth < 8; depth++, node = node.parentElement) { const t = textOf(node); const ps = t.match(priceRe) || []; const rs = node.querySelectorAll?.('a[href*="RatePopup"]').length || 0; if (ps.length === 1 && rs === 1 && t.length < 700) { chosen = t; break; } }
        const parent = el.parentElement; const siblings = []; let s = el.previousElementSibling; for (let j = 0; s && j < 6; j++, s = s.previousElementSibling) siblings.push(textOf(s));
        const combined = [chosen, ...siblings].filter(Boolean).join(' '); const room = roomFrom(combined); const text = chosen || textOf(parent || el);
        return { text, room, board: text.match(boardRe)?.[0] || combined.match(boardRe)?.[0] || '', cancellation: text.match(cancelRe)?.[0] || '' };
      }).catch(() => ({ text: '', room: '', board: '', cancellation: '' }));
      const price = parseDisplayedPrice(info.text || ''); if (!price) continue;
      let room = clean(info.room) || parseRoom(info.text); let boardName = clean(info.board); let cancellation = clean(info.cancellation);
      const best = matchJson(state, room, boardName, search.hotel_name || '');
      if (!room && best) room = best.room; if (!boardName && best) boardName = best.board;
...
async function searchHadafSource(source, search) {
  ...
  try {
    ...
    const end = Date.now() + (Number(cfg.results_wait_ms) || 90000); while (Date.now() < end) { await blocked(page); if (state.jsonResponse && state.json.length) break; await new Promise((resolve) => setTimeout(resolve, 1000)); }
    ...
    const results = await extractRendered(context, cfg, search, state); if (!results.length) throw new Error(`Hadaf returned results but no complete priced rate elements were found. JSON rates: ${state.json.length}. URL: ${page.url()}`);

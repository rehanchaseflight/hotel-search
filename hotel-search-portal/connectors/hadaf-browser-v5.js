...
async function fillSearch(frame, search) {
  const nationality = frame.locator('#lpPannel_sel_nationality'); if (await nationality.count()) await nationality.selectOption({ label: String(search.country || 'United States of America') }).catch(() => {});
  ...
  if (search.hotel_name) { const hotel = frame.locator('#lpPannel_txtHotel').first(); if (await hotel.count()) { const name = String(search.hotel_name); await hotel.evaluate((el, value) => { el.value = value; el.dispatchEvent(new Event('input', { bubbles: true })); el.dispatchEvent(new Event('change', { bubbles: true })); }, name).catch(() => {}); } }
}
...
function flattenJson(value, out = []) {
  if (Array.isArray(value)) { value.forEach((item) => flattenJson(item, out)); return out; }
  if (!value || typeof value !== 'object') return out;
  if (value.n && value.rlst) for (const group of value.rlst || []) for (const rate of group.rdlst || []) out.push({ hotel: clean(value.n), room: clean(rate.rt), board: clean(rate.mp), cancellation: /^(N)$/i.test(String(rate.p || '')) || /^(N)$/i.test(String(rate.bo || '')) ? 'Non-refundable' : '', supplierRoomCode: clean(rate.ThirdPartyRoomCode), rateFrom: clean(rate.fd), rateTo: clean(rate.td) });
...
function makeRate(search, best, room, boardName, price, context) { return { hotel: search.hotel_name || best?.hotel || '', room, view: '', board: boardName, cancellation: cancel(context), price: price.price, currency: price.currency, availability: 'Available', supplier: 'Hadaf Holidays', supplierRoomCode: best?.supplierRoomCode || '', rateFrom: best?.rateFrom || '', rateTo: best?.rateTo || '' }; }
    ...
    for (let i = 0; i < count; i++) {
      ...
      const info = await link.evaluate((el) => {
        ...
        for (let depth = 0; node && depth < 10; depth++, node = node.parentElement) {
          ...
          const links = node.querySelectorAll ? node.querySelectorAll('a[href*="RatePopup"]').length : 0;
          if (prices.length === 1 && links === 1 && boardRe.test(text) && text.length < 1000) { chosen = text; break; }
        }
        return { anchor: (el.innerText || '').replace(/\s+/g, ' ').trim(), container: chosen };
      }).catch(() => ({ anchor: '', container: '' }));

      const text = clean(info.container || info.anchor); const price = parseDisplayedPrice(text); if (!price) continue;
      let room = parseRoom(text); let boardName = board(text); let cancellation = cancel(text);
      const pax = text.match(/Pax:\s*\d+A\d+C\s+(.+)/i); if (pax) room = parseRoom(pax[1]);
...
async function healthHadafSource(source) {
  try { const r = await searchHadafSource(source, { destination: 'Madinah - Saudi Arabia', checkin: '2026-10-02', checkout: '2026-10-03', nights: 1, rooms: 1, guests: 2, children: 0 }); return { configured: r.configured, live: !r.error && r.results.length > 0, error: r.error || null }; } catch (error) { return { configured: true, live: false, error: error.message || String(error) }; }
}

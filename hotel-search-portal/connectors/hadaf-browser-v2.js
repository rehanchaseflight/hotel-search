...
async function bodyText(frame) {
  return clean(await frame.locator('body').innerText().catch(() => ''));
}
...
async function login(page, source, password, cfg) {
  ...
  if (!frame) {
    try {
      await page.goto(cfg.login_frame_url || LOGIN_FRAME, { waitUntil: 'commit', timeout: 20000 });
...
async function openHadaf(source) {
  ...
  try {
    ...
    return { configured: true, live: false, error: e.name === 'TimeoutError' ? `Hadaf browser timed out: ${e.message}` : e.message };
  }
}
...
async function searchForm(page) {
  ...
  while (Date.now() < end) {
    for (const f of page.frames()) {
      const ok = await f.locator('#lpPannel_txtV5City').count().catch(() => 0);
...
async function openSearchPage(page, cfg) {
  ...
  await page.goto(cfg.search_page_url || SEARCH_PAGE, { waitUntil: 'domcontentloaded', timeout: Number(cfg.search_page_timeout_ms) || 30000 });
...
async function chooseDestination(frame, value) {
  ...
  if (!await input.count().catch(() => 0) || !await input.isVisible().catch(() => false)) throw new Error('Hadaf visible destination city input could not be detected');
  ...
  if (!visibleValue || !/makkah|medina|madinah|riyadh|jeddah|saudi/i.test(visibleValue)) throw new Error(`Hadaf destination selection did not stick. Visible value: ${JSON.stringify(visibleValue)}`);
}
...
async function clickSearch(frame, cfg) {
  const exact = frame.locator('a#lpPannel_btnModifySearch').first();
  if (await exact.count().catch(() => 0) && await exact.isVisible().catch(() => false) && /^search$/i.test(clean(await exact.innerText().catch(() => '')))) {
    await exact.click({ timeout: 10000, noWaitAfter: true });
    return 'lpPannel_btnModifySearch';
  }
  if (cfg.search_button_selector) {
    ...
    if (await x.count().catch(() => 0) && await x.isVisible().catch(() => false)) {
      await x.click({ timeout: 10000, noWaitAfter: true });
      return cfg.search_button_selector;
    }
  }
  const controls = frame.locator('a:visible,input:visible,button:visible');
  const n = await controls.count().catch(() => 0);
  for (let i = 0; i < n; i++) {
    const x = controls.nth(i);
    const meta = await x.evaluate(e => ({ tag: e.tagName, type: e.type || '', id: e.id || '', name: e.name || '', value: e.value || '', alt: e.alt || '', title: e.title || '', text: (e.innerText || '').trim(), src: e.src || '' })).catch(() => null);
    if (!meta) continue;
    const label = Object.values(meta).join(' ');
    if (/logout|modify|back|reset|clear/i.test(label) && meta.id !== 'lpPannel_btnModifySearch') continue;
    if (meta.tag === 'A' && /^search$/i.test(clean(meta.text))) { await x.click({ timeout: 10000, noWaitAfter: true }); return meta.id || 'link:Search'; }
    if ((meta.type === 'image' || meta.type === 'button' || meta.type === 'submit') && /search/i.test(label)) { await x.click({ timeout: 10000, noWaitAfter: true }); return meta.id || meta.name || meta.type; }
  }
  throw new Error(`Hadaf search button could not be detected. Controls: ${JSON.stringify(await frame.locator('input:visible,button:visible,a:visible').evaluateAll(els => els.slice(0, 100).map(e => ({ tag: e.tagName, type: e.type || '', id: e.id || '', name: e.name || '', value: e.value || '', alt: e.alt || '', title: e.title || '', text: (e.innerText || '').trim().slice(0, 80), src: e.src || '' }))).catch(() => []))}`);
}
...
function parseHadafRates(payload, cfg = {}) {
  ...
  for (const hotel of hotels) {
    ...
    for (const group of Array.isArray(hotel.rlst) ? hotel.rlst : []) for (const rate of Array.isArray(group && group.rdlst) ? group.rdlst : []) {
  ...
  return dedupeRates(out);
}

function parseDisplayedPrice(text, def = 'AED') {
  const t = clean(text);
  const m = t.match(PRICE_RE);
  if (!m) return null;
  const numeric = clean(m[0]).replace(/,/g, '').match(/([0-9]+(?:\.[0-9]{1,2})?)/);
  const price = Number(numeric && numeric[1]);
  if (!Number.isFinite(price) || price <= 0) return null;
  const currency = (m[0].match(/AED|SAR|USD|EUR|GBP|PKR|US\$|\$/i) || [def])[0];
  return { price, currency: clean(currency) };
}

function roomFromText(text) {
  const t = clean(text);
  const m = t.match(/((?:Standard\s+)?(?:Double|Triple|Twin|Single|Quadruple|Quintuple|Family|Deluxe|Superior|King|Queen|Suite|Apartment|Villa)[^|]*?)(?:\s*-\s*(?:Room Only|Breakfast Included|Bed and Breakfast|Half Board|Full Board|All Inclusive))?(?:\s*$|\s+Non[- ]?refundable|\s+Package Rate)/i);
  return clean(m ? m[1] : '');
}

function boardFromText(text) {
  return clean((clean(text).match(/Room Only|Breakfast Included|Bed and Breakfast|Half Board|Full Board|All Inclusive/i) || [''])[0]);
}

function cancellationFromText(text) {
  return clean((clean(text).match(/Non[- ]?refundable|Free Cancellation|Refundable/i) || [''])[0]);
}

function dedupeRates(rates) {
  const seen = new Set();
  return rates.filter(r => {
    const k = [clean(r.hotel), clean(r.room), clean(r.view), clean(r.board), clean(r.cancellation), r.price, clean(r.currency), clean(r.availability), clean(r.supplierRoomCode), clean(r.rateFrom), clean(r.rateTo), !!r.packageRate].join('|');
...
async function setRecordsPerPage(frame) {
  const selects = frame.locator('select:visible');
  const n = await selects.count().catch(() => 0);
  for (let i = 0; i < n; i++) {
    const s = selects.nth(i);
    const has100 = await s.locator('option').filter({ hasText: /^100$/ }).count().catch(() => 0);
    if (has100) {
      await s.selectOption({ label: '100' }).catch(() => {});
...
async function expandRoomTypes(context) {
  for (const page of context.pages()) for (const frame of page.frames()) {
    const links = frame.locator('a:visible,button:visible').filter({ hasText: /show all room types/i });
    const n = await links.count().catch(() => 0);
    for (let i = 0; i < n; i++) {
      await links.nth(i).click({ timeout: 5000, noWaitAfter: true }).catch(() => {});
      await frame.waitForTimeout(250);
    }
  }
}

async function extractCurrentRenderedRates(context, cfg = {}) {
  await expandRoomTypes(context);
  const out = [];
  for (const page of context.pages()) for (const frame of page.frames()) try {
    const priceLinks = frame.locator('a:visible[href*="HotelResults.aspx#RatePopup"]');
    const count = await priceLinks.count().catch(() => 0);
    for (let i = 0; i < count; i++) {
      const link = priceLinks.nth(i);
      const displayed = await link.innerText().catch(() => '');
      const parsed = parseDisplayedPrice(displayed, cfg.default_currency || 'AED');
      if (!parsed) continue;
      const contextText = clean(await link.evaluate(el => {
        let n = el;
        for (let i = 0; i < 12 && n; i++, n = n.parentElement) {
          const t = (n.innerText || n.textContent || '').replace(/\s+/g, ' ').trim();
          if (t && /Room Only|Breakfast|Half Board|Full Board|All Inclusive|Non-refundable|Package Rate|AED|SAR|USD/i.test(t)) return t;
        }
        return el.innerText || '';
      }).catch(() => displayed));
      let room = roomFromText(contextText);
      if (!room) {
        const before = clean(await link.evaluate(el => {
          let n = el;
          for (let i = 0; i < 8 && n; i++, n = n.previousElementSibling) {
            const t = (n && (n.innerText || n.textContent) || '').replace(/\s+/g, ' ').trim();
            if (t) return t;
          }
          return '';
        }).catch(() => ''));
        room = roomFromText(before);
      }
      let hotel = '';
      try {
        hotel = clean(await link.evaluate(el => {
          let cur = el.closest('div[id],section,article,fieldset') || el.parentElement;
          for (let i = 0; i < 12 && cur; i++, cur = cur.parentElement) {
            const hs = cur.querySelectorAll('h1,h2,h3,h4,.hotelName,.hotel-name,.hotel-title');
            for (const h of hs) {
              const t = (h.innerText || '').replace(/\s+/g, ' ').trim();
              if (t && !/room|price|search/i.test(t)) return t;
            }
          }
          return '';
        }));
      } catch {}
      out.push({ hotel, room, view: '', board: boardFromText(contextText), cancellation: cancellationFromText(contextText), price: parsed.price, currency: parsed.currency, availability: 'Available', supplier: 'Hadaf Holidays', supplierRoomCode: '', rateFrom: '', rateTo: '', packageRate: /Package Rate/i.test(contextText) });
    }
  } catch {}
  return dedupeRates(out);
}

async function collectAllDisplayedRates(context, cfg = {}) {
  const all = [];
  const signatures = new Set();
  const maxPages = Number(cfg.max_result_pages) || 10;
  for (let pageNo = 1; pageNo <= maxPages; pageNo++) {
    const frames = context.pages().flatMap(p => p.frames());
    for (const frame of frames) await setRecordsPerPage(frame).catch(() => false);
    await expandRoomTypes(context);
    const current = await extractCurrentRenderedRates(context, cfg);
    const signature = current.map(r => [r.hotel, r.room, r.price, r.currency, r.cancellation, !!r.packageRate].join('|')).join('||');
    ...
    for (const page of context.pages()) for (const frame of page.frames()) {
      const next = frame.locator('a:visible,button:visible').filter({ hasText: /^Next$/i }).last();
      if (await next.count().catch(() => 0) && await next.isVisible().catch(() => false) && !await next.isDisabled().catch(() => false)) {
  ...
  return dedupeRates(all).slice(0, Number(cfg.max_results) || 1000);
}
...
async function searchHadafSource(source, search) {
  ...
  try {
    ...
    const displayed = await collectAllDisplayedRates(context, cfg);
    let results = displayed.length ? displayed : apiBodies.flatMap(b => { try { return parseHadafRates(JSON.parse(b), cfg); } catch { return []; } });
    ...
    return { configured: true, results: results.slice(0, Number(cfg.max_results) || 1000), error: null };
  } catch (e) {
    return { configured: true, results: [], error: e.name === 'TimeoutError' ? `Hadaf browser timed out at Playwright operation: ${e.message}` : e.message };
  } finally {
    if (context) await context.close().catch(() => {});
    if (browser) await browser.close().catch(() => {});
  }

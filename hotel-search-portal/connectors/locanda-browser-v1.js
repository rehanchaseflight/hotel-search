  ...
  try {
    ...
    const locandaAutocompletePrefix =
      locandaDestinationValue
        ...
        .startsWith('makkah')
        ? 'Mak'
        : locandaDestinationValue.slice(0, 3);
    ...
    /*
     * Give Locanda autocomplete enough time to populate.
     */
    await new Promise(resolve =>
    ...
    const suggestionInfo =
      await page.evaluate((wantedDestination) => {
        const wanted =
          wantedDestination;
        ...
        const match =
          candidates.find(el => {
            ...
            const wantedPrefix =
              wantedBase.slice(0, 3);

            const matchesDestination =
              text.includes(wanted) ||
              textBase.includes(wantedBase) ||
              textBase.startsWith(wantedBase) ||
              (wantedPrefix.length >= 3 &&
               textBase.startsWith(wantedPrefix));

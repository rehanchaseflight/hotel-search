...
function auth(req){const token=parseCookies(req).session;if(!token)return null;try{return jwt.verify(token,JWT_SECRET)}catch{return null}}function allowed(role,...roles){return roles.includes(role)}
...
if(path==='/api/supplier-health'&&method==='GET'){try{const r=await db.query("SELECT id,name,login_url,site_username,site_password_enc,agent_code,connector_type,enabled,browser_config,last_error,last_checked_at FROM sources WHERE enabled=true AND (connector_type='browser' OR connector_type='playwright') ORDER BY name");const statuses=await connectors.healthSources(r.rows);for(const s of statuses){const sourceId=Number(s.id);if(Number.isInteger(sourceId)){await db.query('UPDATE sources SET last_error=$1,last_checked_at=NOW() WHERE id=$2',[s.error||null,sourceId]);}}return json(statuses)}catch(e){console.error(e);return json({error:'Supplier health check failed'},500)}}
 ...
 if(path==='/api/locanda/rates'&&method==='POST'){
  try{
    ...
    if(
      !url ||
      !/^https?:\/\/app\.locandahub\.com\/agent\/booking\/availability\.php\?/i.test(
        String(url)
      )
    ){
      return json({
        ok:false,
        error:'Invalid Locanda availability URL'
      },400);
    }

    const sourceResult=await db.query(
      "SELECT id,name FROM sources WHERE enabled=true AND LOWER(name) LIKE '%locanda%' ORDER BY id LIMIT 1"
    );

    if(!sourceResult.rows[0]){
      return json({
        ok:false,
        error:'Locanda supplier is not enabled'
      },404);
    }

    const locanda=require('./connectors/locanda-browser-v1');

    const result=await locanda.getLocandaRates(
      String(url),
    ...
    console.error('LOCANDA RATES ERROR:',e);

    return json({
      ok:false,
      error:String(e&&e.message||e||'Locanda rates failed')
    },500);
  }
}
...
if(path==='/api/rezlive/details'&&method==='POST'){
  try{
    const{hotelId,roomId,filepostfix}=await body(req);

    if(
      !String(hotelId||'').trim() ||
      !String(roomId||'').trim() ||
      !String(filepostfix||'').trim()
    ){
      return json({
        ok:false,
        error:'RezLive hotel details information is incomplete'
      },400);
    }

    const sourceResult=await db.query(
      "SELECT id,name FROM sources WHERE enabled=true AND LOWER(name) LIKE '%rezlive%' ORDER BY id LIMIT 1"
    );

    if(!sourceResult.rows[0]){
      return json({
        ok:false,
        error:'RezLive supplier is not enabled'
      },404);
    }

    const browserConnector=require('./connectors/browser');

    const result=await browserConnector.getRezLiveHotelDetails({
      hotelId:String(hotelId).trim(),
      roomId:String(roomId).trim(),
      filepostfix:String(filepostfix).trim()
    });

    return json(result);
  }catch(e){
    console.error('REZLIVE DETAILS ERROR:',e);
    return json({
      ok:false,
      error:String(e&&e.message||e||'RezLive hotel details failed')
    },500);
  }
}
if(path==='/api/rezlive/cancellation-policy'&&method==='POST'){
  try{
    const{policyId,roomId,searchId}=await body(req);

    if(!String(policyId||'').trim()){
      return json({
        ok:false,
        error:'Cancellation policy information is incomplete'
      },400);
    }

    const browserConnector=require('./connectors/browser');

    const result=await browserConnector.getRezLiveCancellationPolicy({
      policyId:String(policyId||'').trim(),
      roomId:String(roomId||'0').trim(),
      searchId:String(searchId||'').trim()
    });

    return json(result);
  }catch(e){
    console.error('REZLIVE CANCELLATION POLICY ERROR:',e);
    return json({
      ok:false,
      error:String(
        e&&e.message||e||'Cancellation policy failed'
      )
    },500);
  }
}

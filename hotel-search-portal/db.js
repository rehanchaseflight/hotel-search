...
async function init(){
 ...
 await query(`CREATE TABLE IF NOT EXISTS sources (id SERIAL PRIMARY KEY, name TEXT NOT NULL, login_url TEXT NOT NULL, deep_link_template TEXT, site_username TEXT, site_password_enc TEXT, agent_code TEXT, created_at TIMESTAMPTZ DEFAULT NOW())`);
 for(const sqlText of [`ALTER TABLE sources ADD COLUMN IF NOT EXISTS login_url TEXT`,`ALTER TABLE sources ADD COLUMN IF NOT EXISTS deep_link_template TEXT`,`ALTER TABLE sources ADD COLUMN IF NOT EXISTS site_username TEXT`,`ALTER TABLE sources ADD COLUMN IF NOT EXISTS site_password_enc TEXT`,`ALTER TABLE sources ADD COLUMN IF NOT EXISTS agent_code TEXT`,`ALTER TABLE sources ADD COLUMN IF NOT EXISTS connector_type TEXT NOT NULL DEFAULT 'api'`,`ALTER TABLE sources ADD COLUMN IF NOT EXISTS enabled BOOLEAN NOT NULL DEFAULT TRUE`,`ALTER TABLE sources ADD COLUMN IF NOT EXISTS browser_config JSONB NOT NULL DEFAULT '{}'::jsonb`,`ALTER TABLE sources ADD COLUMN IF NOT EXISTS last_error TEXT`,`ALTER TABLE sources ADD COLUMN IF NOT EXISTS last_checked_at TIMESTAMPTZ`]) await query(sqlText);

 await query(`WITH ranked AS (SELECT id,ROW_NUMBER() OVER (ORDER BY CASE WHEN site_password_enc IS NOT NULL AND site_password_enc<>'' THEN 0 ELSE 1 END,CASE WHEN LOWER(login_url)='https://wanderbeds.com/?setlang=en' THEN 0 ELSE 1 END,id) rn FROM sources WHERE LOWER(TRIM(name))='wanderbeds') DELETE FROM sources WHERE LOWER(TRIM(name))='wanderbeds' AND id IN (SELECT id FROM ranked WHERE rn>1)`);

 await query(`UPDATE sources SET name='WanderBeds',login_url='https://wanderbeds.com/?setlang=en',site_username='newworld',agent_code='WB03437',connector_type='browser',enabled=TRUE,browser_config=CASE WHEN browser_config IS NULL OR browser_config='{}'::jsonb THEN '{"preset":"wanderbeds"}'::jsonb ELSE browser_config END WHERE LOWER(TRIM(name))='wanderbeds'`);

 await query(`INSERT INTO sources(name,login_url,site_username,agent_code,connector_type,enabled,browser_config) SELECT 'WanderBeds','https://wanderbeds.com/?setlang=en','newworld','WB03437','browser',TRUE,'{"preset":"wanderbeds"}'::jsonb WHERE NOT EXISTS (SELECT 1 FROM sources WHERE LOWER(TRIM(name))='wanderbeds')`);

 await query(`CREATE UNIQUE INDEX IF NOT EXISTS idx_sources_name_lower ON sources(LOWER(TRIM(name)))`);
 await query(`INSERT INTO sources(name,login_url,connector_type,enabled,browser_config) SELECT 'Locanda','https://app.locandahub.com/','browser',TRUE,'{"preset":"locanda","requires_manual_captcha":true}'::jsonb WHERE NOT EXISTS (SELECT 1 FROM sources WHERE LOWER(name)='locanda')`);
